...
from BEGE_GARCH.BEGE_GARCH import BEGE_Constant_MLE
...
DIST_PARAM_NAMES = ["shape_p", "shape_n", "sigma_p", "sigma_n"]
...
def load_effective_sample(project_root: Path) -> pd.DataFrame:
    ...
    # Canonical residual definition: u_t = pi_t - hat(pi)_t
...
def _result_to_row(result: dict, mean_type: str, draw: int, seed: int, random_state: int) -> dict:
    ...
    names = MEAN_PARAM_NAMES[mean_type] + DIST_PARAM_NAMES
...
def run_seed(
    ...
    # Effective sample is expected to be fixed at 215 observations.
    if len(df) != 215:
        print(f"[WARN] Effective sample length is {len(df)} (expected 215).")

    _require_columns(df, ["Inflation", "SPF", "SPF_shock"], "BEGE constant runner")
    ...
    print(f"Saved per-seed results to: {out_file}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--n-draws", type=int, default=500, help="Number of random draws per mean specification")
