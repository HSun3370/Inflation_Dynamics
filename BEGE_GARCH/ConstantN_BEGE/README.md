...
This specification is an explicitly requested extension of the baseline BEGE menu. It keeps the bad-news shape process constant while allowing the good-news shape process to follow the unrestricted Full BEGE GJR update:
...
`BEGE_constant_n1.py` estimates the Constant-n Full BEGE specification on the canonical effective sample from `DataSummary/README.md`, **1969Q2--2022Q4** with **215 observations**. The runner uses precomputed lag columns when available, so lagged ARX regressors do not trim the sample again.
...
`collect_constant_n_results.py` merges raw seed files and writes:
...
`SimulationConstantN.sh` defaults to 50 seed jobs, 40 draws per mean process, 25 optimizer starts per draw, and 800 SLSQP iterations per start. This gives 50,000 optimizer starts for each mean process and Constant-n BEGE specification. With all four mean processes, that is 200,000 optimizer starts total.
